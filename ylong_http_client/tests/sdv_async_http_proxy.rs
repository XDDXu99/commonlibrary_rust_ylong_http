...
#![cfg(all(
    feature = "async",
    feature = "http1_1",
    any(feature = "ylong_base", feature = "tokio_base")
))]
...
mod tokio_tests {
    ...
    async fn sdv_http_target_over_http_proxy_tokio() {
        ...
        let proxy = Proxy::http(format!("http://{addr}").as_str())
            .build()
            .unwrap();
        ...
        let mut buf = [0u8; 32];
        loop {
            let read = response.data(&mut buf).await.unwrap();
            if read == 0 {
                break;
            }
            body.extend_from_slice(&buf[..read]);
        }
        assert_eq!(body, b"METHOD GET!");

        let head = observed_rx.await.unwrap();
        assert!(head.starts_with("GET http://target.test:80/data HTTP/1.1\r\n"));
    }
}
